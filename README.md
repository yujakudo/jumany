# JUMAN 7.01拡張API、およびPythonモジュール jumany
JUMAN 7.01に拡張APIを追加するパッチと、そのPythonモジュールです。
現在のところ、Windows 64bitのみ対応しています。
...
PyKNPや標準入出力を介さず、またモジュール内ではライブラリのバッファを
直接参照していますので、幾分高速です。
...
Windows用ビルド済み共有ライブラリを同梱していますので、以降のJUMAN＋拡張APIの
インストールは必用ありません。
...
同梱のWindows用共有ライブラリのライセンスはJUMANのライセンス（3条項BSDライセンス）に準じますが、GNUのライブラリをリンクしているためLGPLが適用されます。
...
## JUMAN＋拡張API
JUMANに拡張APIを追加するパッチと、簡易にビルドするためのスクリプトです。

### 公式リリースとの違い
[公式ページ](http://nlp.ist.i.kyoto-u.ac.jp/index.php?JUMAN)よりダウンロード可能なWindows用バイナリとの仕様上の違いは以下です。
1. 入出力はデフォルトでUTF-8。
1. juman.iniは参照しない。
...
### 制限事項
このパッチと頒布バイナリは、スタンドアローンモードでのみ検証しています。  
サーバ、クライアント、トレーニングの各モードでは検証していません。  
これらのモード内では入力のエンコーディング変換をしていませんので、--encオプションは利用できません。

### ビルド
まず展開ディレクトリにある、ビルド用のスクリプトbtoolを
お好みに合わせて編集してください。

ビルドは、MSYS2のMinGW 64bit上でのみ実績があります。（パッケージ：msys2-x86_64-20161025.exe）
...
名前を変え、適切な場所に移動してください。

#### その他のシステム
LinuxやCygwinでも、btoolを適切に編集すれば、ビルド/インストールは可能かと思います。
